...
const isThisAnEmail = function (str) {
  ...
  if (atIndex === -1 || str.indexOf("@", atIndex + 1) !== -1) {
    return false // più di una @ trovata
  }

  const [beforeAt, afterAt] = str.split("@")

  if (beforeAt.length < 1 || afterAt.length < 1) {
    return false
  }

  // Controlla se ci sono almeno un punto dopo l'@
  if (afterAt.indexOf(".") === -1) {
    return false
  }

  // Verifica che non ci siano spazi bianchi prima o dopo la stringa
  if (str.trim() !== str) {
    return false
  }
...
const howManyDays = function (date) {
  ...
  return console.log(daysDiff, " days has passed from", date)
}

howManyDays("03-14-2024")
/* ESERCIZIO 10
...
const isTodayMyBirthday = function (birthday) {
  ...
  if (birthMonth === todayMonth && birthDay === todayDay) {
    return console.log("Oggi è il tuo compleanno")
  } else {
    return console.log("Oggi non è il tuo compleanno")
  }
}
isTodayMyBirthday("03-15-2024")
// Arrays & Oggetti
...
const newestMovie = function (movies) {
  let newest = movies[0] // Assume il primo film come il più recente all'inizio
...
const searchByTitle = function (searchString, movies) {
  const titleMatch = []

  for (let i = 0; i < movies.length; i++) {
    if (movies[i].Title.toLowerCase().includes(searchString.toLowerCase())) {
      titleMatch.push(movies[i])
    }
  }
...
const searchAndDivide = function (searchString, movies) {
  const obj = {
    match: [],
    unmatch: [],
  }
...
*/
const removeIndex = function (index, movies) {
  if (index < 0 || index >= movies.length) {
    return movies
  }

  return movies.slice(0, index - 1).concat(movies.slice(index))
}
...
const redBackgroundToLink = function () {
...
const addElementInAList = function () {
...
const tree = function (height) {
  for (let i = 1; i <= height; i++) {
    ...
    const row = spaces + leaves + spaces
    console.log(row)
  }
}
...
/*
console.log("Il film più recente nell'elenco è :", newestMovie(movies))
console.log(`Nell'elenco ci sono ${countMovies(movies)} film`)
console.log("Ecco un arrey composto solo dagli anni: ", onlyTheYears(movies))
console.log(
  "I film usciti nell'ultimo millennio sono :",
  onlyInLastMillennium(movies)
)
console.log(
  "La somma di tutti gli anni dei fil in elenco è : ",
  sumAllTheYears(movies)
)
console.log(searchByTitle("Avengers", movies))
console.log(searchAndDivide("Avengers", movies))
console.log("Originale", movies, "Modificato", removeIndex(3, movies))
*/
